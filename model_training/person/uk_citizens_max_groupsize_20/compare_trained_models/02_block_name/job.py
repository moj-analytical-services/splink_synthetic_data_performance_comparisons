...
custom_log.info(f"model path = {model_path}")

# Import settings object used in previous job where we estimated u probabilities
settings_with_u_dict = read_json_from_s3(model_path)

if trial_run:
    settings_with_u_dict["em_convergence"] = 0.003
    settings_with_u_dict["max_iterations"] = 5
else:
    settings_with_u_dict["em_convergence"] = 0.001
    settings_with_u_dict["max_iterations"] = 100

settings_obj = Settings(settings_with_u_dict)

# Add blocking rules, remove corresponding columns


settings_obj.settings_dict["blocking_rules"] = [
    "l.forename1_std = r.forename1_std and l.surname_std = r.surname_std",
]
